...
{
    [DisplayName("dateTimeAsEpochMs"),Description("Parses the input DateTime and outputs as milliseconds since the Epoch (1970-01-01T00:00Z).")]
...
internal static class DateTimeAsEpochMs
{
	internal static void Evaluate(FunctionArgs functionArgs, CultureInfo cultureInfo)
	{
		var dateTimeOffset = DateTimeOffset.ParseExact(
			functionArgs.Parameters[0].Evaluate() as string ?? string.Empty, // Input date as string
			functionArgs.Parameters[1].Evaluate() as string ?? string.Empty,
